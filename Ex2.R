...
MEM_BFGS<-function(times){
  ...
  Variable_Select = matrix(0, p, 5) # 合计五列，分别为：迭代次数，loss，分位数BIC，BIC和AIC
  for(i in 1:p){
    idx = which.min(out[[i]][,1])
    Variable_Select[i, ] = c(idx,out[[i]][idx,])
  }
  VarNum = which.min(Variable_Select[,3])
...
res2_n1_ms1<- sfLapply(1:times, MEM_BFGS)
sfStop()
Lambda2_n1_ms1 = matrix(0, times, p)
Loss2_n1_ms1 = c(rep(0, times))
for (i in 1:times){
  Lambda2_n1_ms1[i, ] = res2_n1_ms1[[i]][-1]
  Loss2_n1_ms1[i] = res2_n2_ms1[[i]][1]
}
...
res2_n2_ms1<- sfLapply(1:times, MEM_BFGS)
sfStop()
Lambda2_n2_ms1 = matrix(0, times, p)
Loss2_n2_ms1 = c(rep(0, times))
for (i in 1:times){
  Lambda2_n2_ms1[i, ] = res2_n2_ms1[[i]][-1]
  Loss2_n2_ms1[i] = res2_n2_ms1[[i]][1]
}
...
res2_n3_ms1<- sfLapply(1:times, MEM_BFGS)
sfStop()
Lambda2_n3_ms1 = matrix(0, times, p)
Loss2_n3_ms1 = c(rep(0, times))
for (i in 1:times){
  Lambda2_n3_ms1[i, ] = res2_n3_ms1[[i]][-1]
  Loss2_n3_ms1[i] = res2_n3_ms1[[i]][1]
}
...
res2_n4_ms1<- sfLapply(1:times, MEM_BFGS)
sfStop()
Lambda2_n4_ms1 = matrix(0, times, p)
Loss2_n4_ms1 = c(rep(0, times))
for (i in 1:times){
  Lambda2_n4_ms1[i, ] = res2_n4_ms1[[i]][-1]
  Loss2_n4_ms1[i] = res2_n4_ms1[[i]][1]
}