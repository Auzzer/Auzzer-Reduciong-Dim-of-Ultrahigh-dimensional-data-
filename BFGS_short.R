...
MSE<-function(lambda, gamma1, X, Y, h, tau,coe){
...
thresold<-function(x,l){
  # l:选择的变量个数
  if(l<p){
    idx = order(x,decreasing = TRUE)[(l+1):p]
    for(item in idx){
      x[idx]=0
    }
    return(x)
...
MEM_BFGS<-function(times){
  ...
  if(VarNum != 1){
    ...
    temp = order(Lambda_final[itNum,],decreasing = TRUE)
    Lambda_temp = c(rep(0,p))
    for(i in 1:p){
      idx = temp[1:i]
      for(j in 1:p){
        if(j %in% idx){Lambda_temp[j] = Lambda_final[itNum,j]}else {Lambda_temp[j] = 0}
      }
      if((OptimCostFunction(Lambda_temp, gamma1, X, Y, h, tau, eps, coe)>0.85*OptimCostFunction(Lambda_final[itNum,], gamma1, X, Y, h, tau, eps, coe))
         & (sum(Lambda_temp)) >(0.85*gamma1)){break}
    }
    loss = MSE(Lambda_temp, gamma1, X, Y, h, tau, coe)
    return(c(loss, Lambda_temp))
  }
